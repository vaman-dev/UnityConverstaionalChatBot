...
namespace LiveKit.Internal
{
    ...
    internal sealed class FfiClient : IFFIClient
    {
        ...
        private const string EditorMissingFfiWarning =
            "[LiveKit] Could not initialize livekit_ffi in the Editor. " +
            "Wait for the automatic FFI download/import to finish, or use " +
            "Convai > Platform Support > Download FFI Libraries (Current Platform).";

        internal enum EditorFfiRetryAction
        {
            None,
            ScheduleRetry,
            LogWarning
        ...
        private static bool _editorInitRetryScheduled;
        private static bool _editorInitGaveUpLogged;
        private static int _editorInitRetryCount;
#endif
        ...
        private static void InitializeSdk()
        {
            ...
            {
                EditorFfiRetryAction action = GetEditorFfiRetryAction(
                    _editorInitRetryCount,
                    _editorInitRetryScheduled,
                    _editorInitGaveUpLogged);
                if (action == EditorFfiRetryAction.ScheduleRetry)
                {
                    _editorInitRetryScheduled = true;
                    EditorApplication.delayCall += RetryEditorInitializeSdk;
                }
                else if (action == EditorFfiRetryAction.LogWarning)
                {
                    _editorInitGaveUpLogged = true;
                    Debug.LogWarning(EditorMissingFfiWarning);
                }
...
#if UNITY_EDITOR
        internal static EditorFfiRetryAction GetEditorFfiRetryAction(
            int retryCount,
            bool retryScheduled,
            bool gaveUpLogged)
        {
            if (retryCount < EditorInitMaxRetries)
                return retryScheduled ? EditorFfiRetryAction.None : EditorFfiRetryAction.ScheduleRetry;

            return gaveUpLogged ? EditorFfiRetryAction.None : EditorFfiRetryAction.LogWarning;
        }

        private static void RetryEditorInitializeSdk()
        {
            _editorInitRetryScheduled = false;
            if (IsOperational || _isDisposed || _editorInitRetryCount >= EditorInitMaxRetries)
                return;
