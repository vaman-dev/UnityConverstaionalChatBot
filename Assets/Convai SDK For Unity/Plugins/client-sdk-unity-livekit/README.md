...
The released Convai package does not bundle `ffi-*` architecture directories. On first import,
the included editor downloader fetches only the current Unity Editor architecture and the
architectures enabled for the active build target. Downloaded plugins are written under
